...
if !!node[:elasticsearch][:basic_auth]
  ...
  cookbook_file "#{node[:elasticsearch][:path][:plugins]}/elasticsearch-http-basic-1.0.4.jar" do
    source "elasticsearch-http-basic-1.0.4.jar"
    owner node[:elasticsearch][:user]
    group node[:elasticsearch][:user]
    mode 0755
    backup false
  ...
  execute "unzip elasticsearch jar" do
    command "unzip #{node[:elasticsearch][:path][:plugins]}/elasticsearch-http-basic-1.0.4.jar -d #{node[:elasticsearch][:path][:plugins]}/http-basic"
